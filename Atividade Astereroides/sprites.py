...
class Ship(pg.sprite.Sprite):
    ...
    def control(self, keys: pg.key.ScancodeWrapper, dt: float, sound=None):
        if keys[pg.K_LEFT]:
        ...
        if keys[pg.K_UP]:
            ...
            if sound and not pg.mixer.get_busy():
                sound.play('thrust')
        elif sound:
            sound.stop('thrust')
...
class UFO(pg.sprite.Sprite):
    def __init__(self, pos: Vec, small: bool):
        ...
        self.fire_cool = uniform(0.5, C.UFO_FIRE_RATE)
        self.target_pos = None

    def fire(self, target_pos: Vec) -> Bullet:
        aim = C.UFO_SMALL["aim"] if self.small else C.UFO_BIG["aim"]
        to_target = (target_pos - self.pos).normalize()
        angle_error = uniform(-1, 1) * (1 - aim) * 45
        rad = math.radians(angle_error)
        cos_a = math.cos(rad)
        sin_a = math.sin(rad)
        dir_x = to_target.x * cos_a - to_target.y * sin_a
        dir_y = to_target.x * sin_a + to_target.y * cos_a

        direction = Vec(dir_x, dir_y)
        vel = direction * C.UFO_BULLET_SPEED

        self.fire_cool = C.UFO_FIRE_RATE
        return Bullet(self.pos, vel)

    def pursue(self, target_pos: Vec):
        self.target_pos = target_pos

    def update(self, dt: float):
        if self.small and self.target_pos:
