    ...
    def run(self):
        while True:
            ...
            for e in pg.event.get():
                ...
                if self.scene.name == "play":
                    if e.type == pg.KEYDOWN and e.key == pg.K_SPACE:
                        self.world.try_fire(self.sound)
                    if e.type == pg.KEYDOWN and e.key == pg.K_LSHIFT:
                        self.world.hyperspace(self.sound)
                elif self.scene.name == "menu":
                    if e.type == pg.KEYDOWN:
                        self.scene = Scene("play")
                        self.world.start_wave(self.sound)
            ...
            if self.scene.name == "menu":
                ...
                self.world.update(dt, keys, self.sound)
